...
public class BloonWorld extends World {
    ...
    // Create transparent overlay to draw roads/sidewalks on
    GreenfootImage overlay = new GreenfootImage(base.getWidth(), base.getHeight());
    overlay.setTransparency(255);

    // Prepare lanes + draw sidewalks/roads onto overlay
    ...
    // Apply as world background
    background = base;
    setBackground(background);

    // Define sidewalk boundaries for monkey spawning
    sidewalkTopStart = 232 - 80;  // top sidewalk begins higher
    sidewalkTopEnd = 232 - 10;
    sidewalkBottomStart = lanePositionsY[laneCount - 1] + (laneHeight / 2) + 10;
    sidewalkBottomEnd = sidewalkBottomStart + 80; // thicker bottom sidewalk
}
    ...
    private void spawnBloons() {
        ...
        for (int lane = 0; lane < laneCount; lane++) {
            ...
            } else if (simulationTime < 4000){
                for (int i = 0; i <= 9; i++) allowedTiers.add(i);
            } else {
                for (int i = 0; i <= 10; i++) allowedTiers.add(i);
            }
            ...
            switch (bloonType) {
                ...
                case 9: b = new RainbowBloon(1, spawner.getY()); break;
                case 10: b = new CeramicBloon(1, spawner.getY()); break;
    ...
    private void spawnMonkeys() {
        ...
        switch (phase) {
            case 1:
                if (roll < 30) monkeyType = 0;
                break;
            case 2:
                if (roll < 25) monkeyType = 1;
                else if (roll < 35) monkeyType = 0;
                break;
            case 3:
                if (roll < 20) monkeyType = 2;
                else if (roll < 30) monkeyType = 1;
                else if (roll < 37) monkeyType = 0;
                break;
        }
    ...
    if (spawnAtTop) {
        ...
        yDespawn = sidewalkBottomStart + sidewalkDepth; // disappear before entering road
        m.setRotation(90); // moving downward
    } else {
        ySpawn = sidewalkBottomStart + sidewalkDepth; // further back from road
        yDespawn = sidewalkTopEnd - sidewalkDepth;   // disappear before entering road
        m.setRotation(270); // moving upward
    }
